...
import { uploadHumanTranscript } from "@/utils/uploadTranscript"
...
export async function POST(
  ...
  try {
    ...
    // Reuse prepare to get normalized transcripts (with empty interviews filtered out)
    ...
    // Check if there are any interviews with messages
    if (!transcriptPayload?.interviews || transcriptPayload.interviews.length === 0) {
      return NextResponse.json({ 
        error: "No interviews with messages found. Please ensure there are completed human interviews with conversations." 
      }, { status: 400 })
    }
    ...
    if (transcriptPayload?.interviews && Array.isArray(transcriptPayload.interviews)) {
      const uploadPromises = transcriptPayload.interviews.map((interview: any) => {
        return uploadHumanTranscript(
          projectId,
          ...
          console.error(`Failed to upload transcript for interview ${interview.respondentId}:`, error);
    ...
    console.error('Error running human analysis:', error)
