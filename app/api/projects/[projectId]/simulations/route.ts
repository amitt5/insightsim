...
export async function GET(
  ...
  try {
    ...
    const { data: projectSimulations, error: simulationsError } = await supabase
      .from("simulations")
      .select(`*`)
    ...
    if (simulationsError) {
      console.error("Error fetching project personas:", simulationsError)
      return NextResponse.json({ error: simulationsError.message }, { status: 500 })
    }

    // Extract personas from the join

    return NextResponse.json({ projectSimulations })
  } catch (error) {
    console.error("Unexpected error:", error)
    return NextResponse.json({ error: "An unexpected error occurred" }, { status: 500 })
  }
